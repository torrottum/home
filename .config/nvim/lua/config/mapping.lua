...
M.setup_gitsigns = function(bufnr)
    ...
    local bufopts = { noremap = true, buffer = bufnr }
    -- Actions
    vim.keymap.set({ "n", "v" }, "<leader>hs", ":Gitsigns stage_hunk<CR>", add_desc("stage hunk"))
    vim.keymap.set({ "n", "v" }, "<leader>hr", ":Gitsigns reset_hunk<CR>", add_desc("Reset hunk"))
    vim.keymap.set("n", "<leader>hS", gs.stage_buffer, add_desc("Stage buffer"))
    vim.keymap.set("n", "<leader>hu", gs.undo_stage_hunk, add_desc("Undo stage hunk"))
    vim.keymap.set("n", "<leader>hR", gs.reset_buffer, add_desc("Reset buffer"))
    vim.keymap.set("n", "<leader>hp", gs.preview_hunk, add_desc("Preview hunk"))
    vim.keymap.set("n", "<leader>gb", function()
        gs.blame_line({ full = true })
    end, add_desc("blame line"))
    vim.keymap.set("n", "<leader>tb", gs.toggle_current_line_blame, add_desc("Gitsigns toggle blame"))
    vim.keymap.set("n", "<leader>gd", gs.diffthis, add_desc("Diff this"))
    vim.keymap.set("n", "<leader>gD", function()
        gs.diffthis("~")
    end, add_desc("Diff this ~"))
    vim.keymap.set("n", "<leader>td", gs.toggle_deleted, add_desc("Gitsigns toggle deleted"))
